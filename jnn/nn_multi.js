...
class NeuralNetwork{
	constructor(x, num_layers , y, z){
		...
		else
		{
			...
			this.weights_h = new Array();
			this.bias_hi = new Array();
			this.weights_ih = new Matrix(this.hidden_nodes, this.input_nodes);
			this.bias_h = new Matrix(this.hidden_nodes, 1);
			this.bias_h.randomize();

			let previous_weights = this.hidden_nodes;
			let previous_bias = this.bias_h;
			for(let i = 0; i< num_layers; i++){
				this.weights_h[i] = new Matrix(this.hidden_nodes, this.input_nodes);
				this.weights_h[i].randomize();
				this.bias_hi[i] = new Matrix(this.hidden_nodes, 1);
				this.bias_hi[i].randomize();
				previous_weights = this.weights_h[i];
				previous_bias = this.bias_hi[i];
			}
	...
	train(input_array, target_array)
	{
		...
		let output_gradients = Matrix.map(dsigmoid);
